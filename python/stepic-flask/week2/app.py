...
def render_main():
    ...
    while current_count < 6:
        current_count += 1
        tours_data[current_count] = tours[random.randint(1, 16)]
...
def render_departure(departure):
    ...
    for index, tour in tours.items():
        if (tour['departure'] == departure):
            count_tour += 1
            tour_by_departure[index] = tour
            if (tour['price'] > max_price):
                max_price = tour['price']
            if (tour['price'] < min_price):
                min_price = tour['price']
            if (tour['nights'] > max_count_night):
                max_count_night = tour['nights']
            if (tour['nights'] < min_count_night):
                min_count_night = tour['nights']
    return render_template('departure.html', departure_name=departures[departure], departure_symbol_code=departure,
                           count_tour=count_tour, max_count_night=max_count_night, min_count_night=min_count_night,
                           max_price=max_price, min_price=min_price, tours_by_departure=tour_by_departure,
                           navigation=departures)


@app.route('/tour/<int:id>/')
def render_tour(id):
    return render_template('tour.html', tour=tours[id], navigation=departures)
